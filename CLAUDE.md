...
3. **Prediction engine** — all math is deterministic TypeScript in `src/lib/prediction-engine.ts` (`buildDeterministicReport`): full Poisson scoreline matrix (0–10 goals) including `mejores_marcadores` (most probable scoreline per outcome), Elo expectation `E = 1/(1+10^(-d/400))` split three ways, and a recency-weighted form/xG panel; percentages use largest-remainder rounding so each method sums to exactly 100. It also emits `regla_empate`: when the absolute Elo difference is below `ELO_PARITY_THRESHOLD` (60) **and** the Poisson draw probability exceeds `POISSON_DRAW_THRESHOLD` (23%), `empate_obligatorio` is true. xG is nullable: when either team lacks xG the form panel reports `xg_disponible: false` with null deltas and the math degrades to Poisson + Elo.

4. **LLM consensus** — `requestConsensus` in `src/pages/api/predict.ts` (`gpt-4o`, `temperature: 0`, JSON mode) only does consensus weighting + textual synthesis over the pre-computed numbers. Output is validated (`validateRules`: integer probabilities summing to exactly 100, winner ∈ {home, away, Empate}, winner/max-probability coherence, draw mandatory when `regla_empate` says so) with one retry that feeds the validation error back. If `empate_obligatorio`, `enforceMandatoryDraw` post-processes the probabilities until draw is strictly highest. **The LLM no longer chooses the scoreline**: after validation, `marcador_exacto` is overwritten with the Poisson `mejores_marcadores` entry matching the predicted winner. Changing the `Prediction` shape requires touching the system prompt's JSON template, the `Prediction` interface (`src/lib/prediction-types.ts`), and `parseStructure` together.

5. **Feedback loop** — `src/lib/feedback-loop.ts#buildTournamentFeedback` compares each finished match's real score against its cached prediction (`src/lib/accuracy.ts#evaluatePrediction`) and produces up to 3 Spanish-language "lessons" per team (full hit / partial hit / error diagnosis with offensive/defensive bias direction). These are appended to the system prompt as an auto-correction section for new predictions involving those teams. Used by both the API endpoint and `npm run precompute`. This is why `npm run clear-cache` preserves finished matches' caches — they are the accuracy history.
